...
class AudioManager {
  ...
  #drawBgMusicButton(context) {
    const bgMusicIsPlaying = this.#bgAudio.currentTime > 0 && !this.#bgAudio.paused
    const img = bgMusicIsPlaying ? this.#musicBtnOnImage : this.#musicBtnOffImage
    ...
    if (img) {
      ...
      const img = wx.createImage()
      bgMusicIsPlaying ? this.#musicBtnOnImage = img : this.#musicBtnOffImage = img
      img.src = bgMusicIsPlaying ? this.#musicBtnOnImageUrl : this.#musicBtnOffImageUrl
      img.onload = () => draw(img)
    }
  }
}
