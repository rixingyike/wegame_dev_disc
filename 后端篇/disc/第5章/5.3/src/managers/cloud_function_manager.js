...
class CloudFunctionManager {
  ...
  /** 查询用户玩家自己的历史游戏数据 */
  async querySelfHistoryDataByPage(pageIndex = 1, pageSize = 10) {
    ...
    if (res && res.errMsg === "cloud.callFunction:ok") {
      const _ = this.#db.command
        , { openid } = res.result
        , listRes = await this.readByPageViaAggregate(HISTORY, pageIndex, pageSize, {
          _openid: _.eq(openid)
        })
      return {
        errMsg: listRes?.errMsg === "readByPageViaAggregate:ok" ? "querySelfHistoryDataByPage:ok" : listRes?.errMsg
        , data: listRes.data
      }
    } else {
      return {
        errMsg: "openid not retrieved"
      }
    }
  }

  /** 原子操作，将玩家自己最近的分数递增1个数字 */
  async increaseSelfLastScore() {
    const res = await wx.cloud.callFunction({
      name: "getUserInfo"
    }).catch(console.log)
    ...
    const _ = this.#db.command
      , { openid } = res.result
      , docRes = await this.#db.collection(HISTORY)
        .where({
          _openid: _.eq(openid)
        })
        .orderBy("dateCreated", "desc")
    ...
    if (docRes && docRes.errMsg === "collection.get:ok") {
      const doc = docRes.data[0]
        , updateRes = await this.#db.collection(HISTORY)
          .doc(doc._id)
          .update({
            data: {
              userScore: _.inc(1)
      ...
      if (updateRes && updateRes.errMsg === "document.update:ok") {
        return {
          errMsg: "increaseSelfLastScore:ok"
          , docId: doc._id
